...
app.post("/register", async (req, res) => {                               // done
  ...
  try {
    ...
    const profilePics = [
      ...
      `${baseUrl}/footballer3.jpg`, 
      `${baseUrl}/footballer4.jpg`
    ];
...
app.post("/picUpload", async (req, res) => {
  const { token } = req.body;
  const newpath = process.env.UPLOAD_PATH || (__dirname + "/public/profiles/");
    ...
    if (user == "token expired") {
      ...
      file.mv(`${newpath}${filename}`, (err) => {
        if (err) {
          ...
          const baseUrl = process.env.BASE_URL || 'http://127.0.0.1:5000';
          res.status(200).send({ message: "File Uploaded", data: `${baseUrl}/profiles/${filename}`, code: 200 });
        }
      });
    }
...
app.post("/getEvents", async (req, res) => {
  ...
  try {
    ...
    const user_id = userId ? userId : userData._id;

    const userEvents = await UserEvents.find({ user: user_id, status: status });
    ...
    for (let event of events) {
      ...
      let sportInfo = null;
      if (teamInfo && teamInfo.sportsType) {
        sportInfo = await Sports.findById(teamInfo.sportsType);
      }
      
      let eventData = {
        id: event._id,
        name: event.name,
        sport: sportInfo ? sportInfo.name : 'Unknown Sport',
    ...
    console.log(error);
    res.send({ status: "error", data: error.message || "An error occurred" });
  }
});
...
app.post("/addSportsType", async (req, res) => {
  ...
  // Use environment variable for admin key validation
